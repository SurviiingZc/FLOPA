...
module qkv_tile_cache #(
  ...
  wire k_next_valid_w;
  wire v_next_valid_w;
  wire [1:0] q_bank_valid_w;
  wire [1:0] k_bank_valid_w;
  wire [1:0] v_bank_valid_w;

  // Exactly one active bank responds per tensor, so valid safely selects its data.
  assign load_ready_o = (load_kind_i == `ATTN_CACHE_Q) ?
                        !q_bank_valid_w[load_bank_i] :
                        (load_kind_i == `ATTN_CACHE_K) ?
                        !k_bank_valid_w[load_bank_i] :
                        (load_kind_i == `ATTN_CACHE_V) ?
                        !v_bank_valid_w[load_bank_i] : 1'b0;
  ...
  assign kv_active_valid_o = k_active_valid_w & v_active_valid_w &
                             (k_active_bank_w == v_active_bank_w);
  ...
  pingpong_buffer u_q_pp (
    ...
    .active_valid_o(q_active_valid_o), .next_valid_o(q_next_valid_o),
    .bank_valid_o(q_bank_valid_w),
  ...
  pingpong_buffer u_k_pp (
    ...
    .active_valid_o(k_active_valid_w), .next_valid_o(k_next_valid_w),
    .bank_valid_o(k_bank_valid_w),
  ...
  pingpong_buffer u_v_pp (
    ...
    .active_valid_o(v_active_valid_w), .next_valid_o(v_next_valid_w),
    .bank_valid_o(v_bank_valid_w),
