...
(* keep_hierarchy = "yes" *) module fsa_stripe #(
  ...
  generate
    ...
    for (boundary_col = 0; boundary_col < COLS;
      ...
      assign k_bottom_last_o[boundary_col] =
          k_last_w[STRIPE_ROWS][boundary_col];
    end

    genvar reduce_row;
    for (reduce_row = 0; reduce_row < STRIPE_ROWS;
         reduce_row = reduce_row + 1) begin : g_reduce_boundary
      assign max_valid_w[reduce_row][0] = pe_last_w[reduce_row][0] && !ws_pv_q;
    ...
    for (pe_row = 0; pe_row < STRIPE_ROWS; pe_row = pe_row + 1) begin : g_pe_row
      for (pe_col = 0; pe_col < COLS; pe_col = pe_col + 1) begin : g_pe_col
        fsa_fused_pe #(
          ...
          .clear_score_i(clear_score_i), .ws_pv_i(ws_pv_q),
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      delta_group_index_q <= {DELTA_GROUPS*COL_IDX_W{1'b0}};
      delta_group_data_q <= {DELTA_GROUPS*STRIPE_ROWS*SCORE_W{1'b0}};
