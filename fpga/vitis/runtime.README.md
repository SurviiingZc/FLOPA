...
`BUILD_INFO` records the Git commit, clean/dirty state, source-diff digest,
requested kernel frequency, platform, and toolchain. `SOURCE.diff` preserves the
tracked working-tree changes used for the build. Both are covered by `SHA256SUMS`.
...
loaded memory tiles. The final board run reported `2942` cycles, zero stall cycles, and zero
errors both before and after the model workload.
