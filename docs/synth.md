...
### S2: Phase-local RTL clock gating - implemented, pending power synthesis

- ASIC：22 个显式 RTL ICG，包括 array control/exp、Q/PV-seed/KV skew、每
  stripe 的 control/delta、完整 PE/O-bank/O-seed pipeline，以及
  normalizer/output buffer。
- DC `-gate_clock` 固定关闭，禁止 `SNPS_CLOCK_GATE_HIGH_*` 自动包装和悬空 TE。
- 门控 enable 覆盖 start、最大深度 occupancy/PE drain counter、同步 clear 和
  normalization/writeback，不能只检测单周期 input valid。
...
功耗复测必须使用 512x512、seed 301、1.6 ns 门级 SAIF；clock network dynamic
硬门槛为不高于 0.18325 W。完整经验与签核顺序见
[clock_gating.md](clock_gating.md)。
...
- 默认 mapped 综合不启用 `set_fix_hold`；仅 physical-aware FF/min-RC
  `prects-hold` 启用并执行 hold-focused incremental mapping。
